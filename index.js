...
(async () => {
    ...
    let results = await processInBatches(ipAddresses, config.batchSize, config.maxConcurrentLookups);
...
results = await Promise.all(results.map(async (result) => {
    if (result.status === 'Success') {
        try {
            ...
            result.sslCertificate = { issuer: 'N/A', validFrom: 'N/A', validTo: 'N/A' };
            result.headers = { server: 'N/A', xPoweredBy: 'N/A', location: 'N/A', securityHeaders: {} };
        }
    } else {
        result.sslCertificate = { issuer: 'N/A', validFrom: 'N/A', validTo: 'N/A' };
        result.headers = { server: 'N/A', xPoweredBy: 'N/A', location: 'N/A', securityHeaders: {} };
